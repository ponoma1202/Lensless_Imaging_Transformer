...
full_data_list_target = [f for f in os.listdir(target_dir) if f.endswith('.tiff') or f.endswith('.tif')]
...
if dataset == 'mirflickr':
    ...
    train_set = sorted_data_list_data[5000:int(percentage * 100000)]      # just want to use 50k images for training
    val_set = sorted_data_list_data[1000:5000]
    test_set = sorted_data_list_data[:1000]
    train_set_target = sorted_data_list_target[5000:int(percentage * 100000)]      # make two separate lists if they use different file naming conventions
