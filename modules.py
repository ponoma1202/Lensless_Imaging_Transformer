...
class OverlapPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size, patch_size, stride, in_chans, embed_dim):
        ...
        img_size = to_2tuple(img_size)          # duplicates img_size and creates tuple (img_size, img_size)
...
class AxialAttention(nn.Module):
    def __init__(self, in_planes, out_planes, groups, kernel_size,
        ...
        relative_index = key_index - query_index + self.kernel_size - 1  # (14,14)
    ...
    def forward(self, x):
        ...
        N, W, C, H = x.shape
        x = x.contiguous().view(N * W, C, H)        # N = batch size

        # Transformations
        qkv = self.bn_qkv(self.qkv_transform(x))
        q, k, v = torch.split(qkv.reshape(N * W, self.groups, self.group_planes * 2, H),            # split into 3 tensors with self.group_planes // 2 or self.group_planes number of heads
                              [self.group_planes // 2, self.group_planes // 2, self.group_planes], dim=2)

        # Calculate position embedding. From self.relative vector, select all columns (dim = 1) corresponding to flatten_index.
...
class Encoder(nn.Module):
    def __init__(self, input_size=1600, in_chans=1, embed_dims=[64, 128, 256, 512], num_heads=[1, 2, 4, 8], depths=[2, 2, 2, 2]):
        ...
        H4, W4 = get_feat_size(H3, W3, 2)
        self.patch_embed1 = OverlapPatchEmbed(img_size=input_size, patch_size=7, stride=4, in_chans=in_chans,
                                              embed_dim=embed_dims[0])
        self.patch_embed2 = OverlapPatchEmbed(img_size=(H1, W1), patch_size=3, stride=2, in_chans=embed_dims[0],
                                              embed_dim=embed_dims[1])
        self.patch_embed3 = OverlapPatchEmbed(img_size=(H2, W2), patch_size=3, stride=2, in_chans=embed_dims[1],
                                              embed_dim=embed_dims[2])
        self.patch_embed4 = OverlapPatchEmbed(img_size=(H3, W3), patch_size=3, stride=2, in_chans=embed_dims[2],
                                              embed_dim=embed_dims[3])

        # encoder
        self.block1=[]
        for _ in range(depths[0]):
            self.block1.append(Encoder_Block(dim=embed_dims[0], heads=num_heads[0], kernel_size=(H1, W1)))
        self.block1=nn.Sequential(*self.block1)
        self.norm1 = nn.BatchNorm2d(embed_dims[0])

        self.block2=[]
        for _ in range(depths[1]):
            self.block2.append(Encoder_Block(dim=embed_dims[1], heads=num_heads[1], kernel_size=(H2, W2)))
        self.block2=nn.Sequential(*self.block2)
        self.norm2 = nn.BatchNorm2d(embed_dims[1])

        self.block3=[]
        for _ in range(depths[2]):
            self.block3.append(Encoder_Block(dim=embed_dims[2], heads=num_heads[2], kernel_size=(H3, W3)))
        self.block3=nn.Sequential(*self.block3)
        self.norm3 = nn.BatchNorm2d(embed_dims[2])

        self.block4=[]
        for _ in range(depths[3]):
            self.block4.append(Encoder_Block(dim=embed_dims[3], heads=num_heads[3], kernel_size=(H4, W4)))
...
class Decoder(nn.Module):
    def __init__(self, sum_encoder_embed_dims=960,rec_channels=1, rec_size=500):
        ...
        self.rec_size = rec_size        # now a tuple
